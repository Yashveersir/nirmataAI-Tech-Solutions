...
export function ServiceCard({ service, icon: Icon = Code2, className }: ServiceCardProps) {
  return (
    <Card
      className={cn(
        "group h-full bg-card border-border flex flex-col justify-between overflow-hidden relative shadow-sm transition-all hover:shadow-md hover:-translate-y-1",
        className,
      )}
    >
